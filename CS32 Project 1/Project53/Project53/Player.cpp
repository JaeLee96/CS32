...
bool Player::shoot(int dir)
{
	...
	if (rand() % 3 == 0) {
		m_arena->thePast().markIt(m_row, m_col);// miss with 1/3 probability
		return false;
	}
	...
	switch (dir)
	{
	case UP:     rDelta = -1; m_arena->thePast().markIt(m_row, m_col); break;
	case DOWN:   rDelta = +1; m_arena->thePast().markIt(m_row, m_col); break;
	case LEFT:   cDelta = -1; m_arena->thePast().markIt(m_row, m_col); break;
	case RIGHT:  cDelta = +1; m_arena->thePast().markIt(m_row, m_col); break;
